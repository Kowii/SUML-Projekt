...
def main():
    ...
    logger.info(f"Writing classes mapping to: {classes_path}")
    with open(classes_path, "w") as f:
        json.dump(classes, f, indent=2)
    logger.info("Class mapping successfully saved.")
    
    # 2. Instantiate and save model weights
    logger.info(f"Instantiating pretrained model '{model_name}' using timm...")
    try:
        ...
        logger.info(f"Saving model state dictionary to: {model_path}")
        ...
        logger.error(f"Error occurred while generating dummy model: {str(e)}", exc_info=True)
        sys.exit(1)
